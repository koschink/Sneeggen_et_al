...
g = sns.boxplot(x="Genotype", y="Lifetime", data=df4, dodge=False, palette=pal1)
g = sns.stripplot(x="Genotype", y="Lifetime", hue="Experiment", data=df4, dodge=False)
plt.ylabel("tubule lifetime (s)")
plt.show()

means = df4.groupby(["Experiment", "Genotype"]).mean()
df_means = means.reset_index()

cat1_wt = df_means[df_means['Genotype']=='WT']
cat1_KO = df_means[df_means['Genotype']=='KO']

print("T-test on means")

stats = ttest_ind(cat1_wt['Lifetime'], cat1_KO['Lifetime'])
