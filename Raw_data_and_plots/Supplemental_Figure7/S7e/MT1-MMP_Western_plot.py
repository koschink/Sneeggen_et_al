...
csvname = "MT1_MMP_KD_RPE_Western.csv"
epsname = "MT1_MMP_KD_RPE_Western.eps"
...
ax=sns.barplot(x="Treatment", y="MT1-MMP (%)", data=df, capsize=0.3)
ax= sns.swarmplot(x="Treatment", y="MT1-MMP (%)", data=df, palette=pal, size=10)
...
ax=sns.pointplot(x="Treatment", y="MT1-MMP (%)", data=df, join=False, capsize=0.3, color="Black")
ax= sns.swarmplot(x="Treatment", y="MT1-MMP (%)", data=df, palette=pal, size=10)
for tick, text in zip(ax.get_xticks(), ax.get_xticklabels()):
    sample_name = text.get_text()  # "X" or "Y"

    # calculate the mean value for all replicates of either X or Y
    mean_val = df[df['Treatment']==sample_name]["MT1-MMP (%)"].mean()

    # plot horizontal lines across the column, centered on the tick
    ax.plot([tick-mean_width/3, tick+mean_width/3], [mean_val, mean_val],
            lw=4, color='black')
ax.tick_params(labelsize=14)
ax.set_ylabel("relative MT1-MMP expression (%)", fontsize = 12)
ax.set_xlabel("")
...
cat1_KO = df[df['Treatment']=='siMT1-MMP']

stats = ttest_1samp(cat1_KO['MT1-MMP (%)'], 100)
