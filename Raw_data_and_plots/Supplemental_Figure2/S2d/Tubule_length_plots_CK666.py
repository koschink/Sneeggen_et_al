...
filename = "Tubule_length_CK666_tidy.csv"
...
pal = sns.color_palette("viridis", 4)
g = sns.pointplot(y="tubule length", x="Treatment", data=df3, order=["control","CK666"], join=False)
g = sns.swarmplot(y="tubule length", x="Treatment", hue="Experiment", data=df3, order=["control","CK666"], palette=pal)

plt.show()


cat1_wt = df3[df3['Treatment']=='control']
cat1_KO = df3[df3['Treatment']=='CK666']
...
ax=sns.pointplot(x="Treatment", y="tubule length", data=df3, order=["control","CK666"], join=False, capsize=0.3, color="black", markers="_" )
ax= sns.swarmplot(x="Treatment", y="tubule length", data=df3, order=["control","CK666"], size=10)
